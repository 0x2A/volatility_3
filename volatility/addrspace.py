...
class BaseAddressSpace(object):
    ...
    def _set_profile(self, profile_name):
        ...
        if profile_name == None:
            raise AssertionError("You must set a profile!")
        if profile_name in PROFILES:
            ...
            if profile_name in profs:
                ...
                raise AssertionError("Invalid profile " + profile_name + " selected")
        if not self.is_valid_profile(ret):
            raise AssertionError("Incompatible profile " + profile_name + " selected")
        return ret
